<template>
	<div class="com-page">
        <a href="###" class="btn-turnpage page-prev" v-if="pageParams.nowPage != 1">上一页</a>
        <a href="###" class="btn-num" v-if="showFirstPage">1</a>
        <span class="more" v-if="showFirstPage">...</span>
        <a href="###" class="btn-num" v-for="item in pagelist" :class="{'on':pageParams.nowPage == item}">{{item}}</a>
        <span class="more" v-if="showLastPage">...</span>
        <a href="###" class="btn-num" v-if="showLastPage">{{pageParams.totalPage}}</a>
        <a href="###" class="btn-turnpage page-next" v-if="pageParams.nowPage != pageParams.totalPage">下一页</a>
...
export default {
    props: ['pageParams'], //pageParams结构：{pageSize: 10, nowPage: '', totalPage: ''}
    data () {
    	return {
            current: 1, //当前页
            allPage: 0, //总页数
    ...
    computed: {
        ...
        pagelist: function(){ //分页列表
            ...
            if(this.allPage > this.showItem) { //总页数大于要显示的页数，出现省略号
                if(_params.nowPage - _bt <= 3) { //只显示尾部省略号
                ...
                else if(_params.nowPage + _bt >= _params.totalPage - 2) { //只显示头部省略号
                    this.showFirstPage = true;
                    this.showLastPage = false;
                    _num = _params.totalPage;
                    while(_num >= _params.totalPage - (this.showItem - 3)) {
                        _list.unshift(_num--);
                    }
                }
                else { //头部尾部省略都显示
                    this.showFirstPage = true;
                    this.showLastPage = true;
                    _num = _params.nowPage
                }
            }
            if(_params.nowPage < this.showItem) { //当前页小于要显示的页数
                this.showFirstPage = false;
                this.showLastPage = false;
                ...
                this.pageParams.nowPage = _page.nowPage;
                while(i)
            }
        }
        // pagelist: function(){ //分页列表
